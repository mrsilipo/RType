...
public static class PerformanceProbe
{
    public static void Run(GameLaunchOptions options)
    {
        ...
        if (parameters.Audio.EngineSimulatorEnabled && parameters.Audio.EngineSimulatorVolume > 0f)
        {
            ...
            MeasureEngineSimAudioFluidStepVariants(parameters);
        }
    }
    ...
    private static void MeasureEngineSimAudioFluidStepVariants(VehicleSimulationParameters parameters)
    {
        foreach (int fluidSteps in new[] { 1, 2, 4 })
        {
            MeasureEngineSimAudio(parameters, fluidSteps);
        }
    }

    private static void MeasureEngineSimAudio(VehicleSimulationParameters parameters, int fluidSteps)
    {
        const int sampleRate = 44100;
        const int seconds = 3;
        const int frames = sampleRate * seconds;
        EngineSimulatorSampleSynth synth = new(parameters.Audio, sampleRate, fluidSimulationStepsOverride: fluidSteps);
        synth.SetTarget(new EngineSimulatorSynthesisTarget(
            7200f,
            1f,
            0.96f,
        ...
        stopwatch.Stop();
        double realtimeRatio = seconds / Math.Max(0.001, stopwatch.Elapsed.TotalSeconds);
        Console.WriteLine($"  engine audio synth fluid {fluidSteps}: {seconds:0.0}s in {stopwatch.Elapsed.TotalMilliseconds:0.0} ms, {realtimeRatio:0.00}x realtime, peak {peak:0.000}");
    }
