...
The next report showed stable stream depth but the audible result was still described as noisy/static. The imported MR presentation values (`jitter 0.195`, `noise 0.253`) were too aggressive for the managed realtime output path, and the leveler could boost peaks to 1.9x around a 30,000 sample target. Realtime jitter/noise presentation is now softened to 10%/12% of those values and the leveler has 24,000 target headroom with a 1.25x maximum gain. The combustion and gas-flow signal is unchanged.

The following live run still had no empty queue, but hybrid exhaust convolution occasionally took 20-25 ms per 11.6 ms buffer and produced audible grain during load changes. Live playback now limits the impulse response to the first 512 direct taps. The longer tail remains available in the asset/profile for offline fidelity work, while realtime combustion avoids that additional CPU and tail smearing.

The direct-response run still sounded choppy/static, so the realtime presentation path is now clean-output mode: no jitter, air-noise, derivative mix, convolution, intake/transient/driveline post-layers, or output clipping boost. The Engine Sim gas-flow, combustion, cam/VTEC, limiter, and exhaust-channel generator remain the source signal. This isolates the simulator from optional DSP artifacts before those layers are reintroduced one at a time.
